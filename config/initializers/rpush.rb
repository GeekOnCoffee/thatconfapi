Rpush.configure do |config|
  ...
  # The maximum number of notifications to load from the store every `push_poll`
  # seconds. If some notifications are still enqueued internally, Rpush will
  # load the batch_size less the number enqueued. An exception to this is if the
  # service is able to receive multiple notification payloads over the
  # connection with a single write, such as APNs.
  ...
  # rubocop:disable Style/TernaryParentheses, Metrics/LineLength
  config.log_level = (defined?(Rails) && Rails.logger) ? Rails.logger.level : ::Logger::Severity::INFO
  # rubocop:enable Style/TernaryParentheses, Metrics/LineLength
...
Rpush.reflect do |on|
  ...
  # on.notification_id_failed do |app, notification_id,
  ...
  # Called when the GCM returns a failure that indicates an invalid registration
  # id. You will need to delete the registration_id from your records.
  ...
  on.ssl_certificate_will_expire do |_app, _expiration_time|
    ApplicationMailer.support_email(
      'Rpush certificate expiring in one month',
      'The Apple Push Notification Service SSL certificate for Binary will ' \
      'expire in one month',
    )
  end

  # Called when an SSL certificate has been revoked.
  on.ssl_certificate_revoked do |_app, _error|
    ApplicationMailer.support_email(
      'Rpush certificate expired',
      'The Apple Push Notification Service SSL certificate for Binary has ' \
      'expired. A new certificate must be configured before push ' \
